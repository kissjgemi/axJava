...
public class Line extends Thread {
    ...
    private double setDeltaC() {
        return Math.sqrt((deltax * deltax + deltay * deltay));
    }

    private double setMyArcus() {
        return Math.atan2((double) ry - sy, (double) rx - sx);
    }
...
//            drawY = (int) (startY + s * deltay);
            drawX = (int) (startX + s * deltac * Math.cos(myArcus));
            drawY = (int) (startY + s * deltac * Math.sin(myArcus));
            if (s == steps) {
                drawX = this.rx;
                drawY = this.ry;
            }
